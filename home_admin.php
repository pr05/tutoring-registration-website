...
$sql = "SELECT class.id, subject, description, day, time, duration, no_students, first_name, last_name, email, grade, status.type,
        ...
        WHERE class.schedule_id = schedule.id AND class.status = status.id AND day_id = day_type.id AND class.tutor_id = user.id ORDER BY day_id";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr>"
                ...
                . "<td>".$row["grade"]."</td>"
                . "<td>".$row["type"]."</td>"
                . "<td><a href=\"home.php?col=change_class_status&id=".$row["id"]."&status=".$row["type"]."\"><input type=\"button\" value=\"Change Status\"></a></td>"
                . "</tr>";
    }
    echo "</table>";
}
